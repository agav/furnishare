  ...
  def initialize(c, name, subAssemblyName, material,metricVolume)
    ...
    c.entities.each do |entity|
      if entity.is_a? Sketchup::Face
        entity_dimensions = getSortedArray([entity.bounds.height, entity.bounds.width, entity.bounds.depth])
        entity_thickness = entity_dimensions[1]
        if entity_thickness == @thickness
          if entity.material
           puts entity.material.display_name.to_s
          end
        end
      end
    end

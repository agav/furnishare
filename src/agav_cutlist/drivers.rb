...
class HtmlLayoutDriver < HtmlOutputDriver
  ...
  def renderLayoutBoards(layoutBoards,type)
    ...
    if ( layoutBoards != nil && layoutBoards.length > 0 )
      ...
      @html += @renderer.displayEfficiency(AgavCutList::string_to_html(getTotalEfficiencyString))
      
      # display board feet required for layout ( # of boards * bdft for each board)
      calculateTotalLayoutBoardFeet(layoutBoards)
      @html += @renderer.displayBoardFeet(AgavCutList::string_to_html(getTotalBoardFeetString(layoutBoards)))
  ...
  def render
    ...
    #debug
    puts "unplacedParts is nil" if (@unPlacedParts == nil ) && AgavCutList.verbose1
    puts "unplacedParts is empty" if  (@unPlacedParts != nil ) && (@unPlacedParts.empty? ) && AgavCutList.verbose1
    #display any uplaced parts
    if @unPlacedParts != nil  && !@unPlacedParts.empty?
      puts "unplaced parts list has " + @unPlacedParts.getList.length.to_s + " entries" if AgavCutList.verbose1
  ...
  def showUnPlacedParts(unPlacedParts)
    ...
    unPlacedPartsList.each { |part|
      @html += @renderer.displayText(@x, @y, AgavCutList::string_to_html(part.summary))
      pageYIncrement(20)
    }
  end
  ...
  def calculateTotalLayoutBoardFeet(layoutBoards)
    if layoutBoards != nil
      ...
      @layoutBoardFeet = AgavCutList::float_round_to(4, @layoutBoardFeet)
    end
  end

  def getTotalBoardFeetString( layoutBoards )
    if ( layoutBoards != nil && layoutBoards.length > 0 )
      return "Total used for layout: " + AgavCutList::decimal_to_comma(@layoutBoardFeet.to_s) + " " + ( layoutBoards[0].board.getBoardFeetUnitsString )
    else
      return ""
    end
  end
  ...
  def calculateUtilisation(layoutBoards)
    ...
    if layoutBoards != nil
      ...
      layoutBoards.each { |layoutBoard|
        ...
        @totalEfficiency = AgavCutList::float_round_to(2, (efficiency/x))
      }
    end
  end
  
  def getTotalEfficiencyString
    return "Efficiency = " + AgavCutList::decimal_to_comma(@totalEfficiency.to_s) + "%"
  end
  ...
  def renderBoard(layoutBoard)
    ...
    @html += @renderer.drawBoard(2, "#cacaaf", AgavCutList::string_to_html(layoutBoard.to_s), @x, @y, board.getLengthPx, board.getWidthPx)
  ...
  def renderPart(part)
    ...
    @html += @renderer.drawPart(2, "#bcbc9a", AgavCutList::string_to_html(part.getName), @x+coords[0], @y+coords[1], part.getLengthPx, part.getWidthPx)
  end

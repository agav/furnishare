...
class Reporter
  ...
  def layout
      ...
      if !@solidPartList.empty? && @@options[:cutlist_Options][:showComps]
        ...
        @listOfSolidPartsLists.each { |solidPartList|
          ...
          puts "Boards Added=" + @layoutBoards.size.to_s if AgavCutList.verbosePartPlacement
          @unplacedPartsList +(unplacedPartsList) if unplacedPartsList != nil
          puts "Unplaced Solid Parts during layout=" + unplacedPartsList.count.to_s if  AgavCutList.verbosePartPlacement
        }
      end
      ...
      if !@sheetPartList.empty? && @@options[:cutlist_Options][:showSheet]
        ...
        @listOfSheetPartsLists.each { |sheetPartList|
          ...
          puts "Sheets Added=" + @layoutSheets.size.to_s if AgavCutList.verbosePartPlacement
          @unplacedPartsList +(unplacedPartsList) if unplacedPartsList != nil
          puts "Unplaced Sheet Parts during layout=" + unplacedPartsList.count.to_s if  AgavCutList.verbosePartPlacement
        }
     end
  ...
  def getSubComponents(entityList, level, subAssemblyName)
    puts "checking level=" + level.to_s if AgavCutList.verboseComponentDiscovery
      ...
      if ( (inSelection || level>1) && c.layer.visible? && !c.hidden?)
        ...
        if ( (c.is_a? (Sketchup::ComponentInstance)) || (c.is_a? (Sketchup::Group)) )
          ...
          if c.is_a? (Sketchup::ComponentInstance) 
            ...
            puts "component instance with definition name=" + compName.to_s if AgavCutList.verboseComponentDiscovery
          elsif c.is_a? (Sketchup::Group)
            compName = c.name
            puts "group with name=" + compName.to_s if AgavCutList.verboseComponentDiscovery
            if (compName == nil || compName == "" )
		compName = AgavCutList.group_definition(c).name
              #let's see if this is a copy of a group which might already have a name
              if ( compName != nil && compName != "" )
                puts "group had no name but is assigned name=" + compName.to_s + " based on its parent" if AgavCutList.verboseComponentDiscovery
              end
            end
          end ##if
          ...
          if (isPartOrSheet( @@options[:cutlist_Options][:partWords], partMaterial ) ||
            ...
            puts "adding part name=" + compName.to_s + " level=" + level.to_s  + " as a hardware part since material or name matched"  if AgavCutList.verboseComponentDiscovery
            puts "+++++++++++++++++++++++++++" if AgavCutList.verboseComponentDiscovery
          ...
          if (!hasSubComponents )
           puts "adding part name=" + compName.to_s + ",subAssembly=" + subAssemblyName.to_s + " level=" + level.to_s  + " since level=" + (level+1).to_s + " has no subcomponents" if AgavCutList.verboseComponentDiscovery
           puts "+++++++++++++++++++++++++++" if AgavCutList.verboseComponentDiscovery
            ...
            puts "skipping partname=" + compName.to_s + " at level=" + level.to_s  + " since level=" + (level+1).to_s + " has subcomponents" if AgavCutList.verboseComponentDiscovery
            puts "--------------------------" if AgavCutList.verboseComponentDiscovery
          end
    ...
    puts "returning levelHasSubcomponents=" + levelHasComponents.to_s + " for level=" + level.to_s if AgavCutList.verboseComponentDiscovery
  ...
  def components    
	...
	puts "Component Discovery start ---->" if AgavCutList.verboseComponentDiscovery
	# pass the selection, not the entities
	getSubComponents(selection,1, @mname)
	#DEBUG
	puts "Component Discovery end <----" if AgavCutList.verboseComponentDiscovery

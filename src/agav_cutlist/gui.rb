...
class GuiBase
  ...
  @@title = "Cut List " +  AgavCutList.version
...
#    @dlg.set_size(600,550)
  ...
  def addCallbacks
    ...
    @dlg.add_action_callback("handleSaveConfig"){|d,p| 
      ...
      $stdout = stdout
      puts "Saved to " + File.dirname(__FILE__)+getConfigLocation if AgavCutList.verbose1
    }
  end
  ...
  def calcKerfSize(kerfSizeStr, kerfSizeUnits)
    ...
    if (kerfSizeUnits == "32nd")
      ...
      return AgavCutList::float_round_to(4, Float(kerfSize/25.4)).inch
    else
       return 0
    end
  end
  ...
  def parse_input_string(p)
    ...
    puts "input parameter array: <<<<<"  + p.to_s if AgavCutList.verboseParameters
    a = p.split(',')
    parse_input_array(a)
    @cutlist_options = getCutlistOptions(a)
    # display the cutlist options after they are parsed from the input parameters
    puts "cutlist options parsed: <<<<<"  + @cutlist_options.to_s if AgavCutList.verboseParameters
    @layout_options = getLayoutOptions(a)
    # display the layout options after they are parsed from the input parameters
    puts "layout options parsed: <<<<<"  + @layout_options.to_s if AgavCutList.verboseParameters
  end

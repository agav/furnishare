...
class Display < Display_class
  ...
  def processRows(inList)
    ...
    for c in inList
      ...
      ix = AgavCutList::integer_to_fws(3, i) + "-" + AgavCutList::integer_to_fws(2, ii)
      ...
      cols[6]=AgavCutList::decimal_to_comma(c.getBoardFeet.to_s)
  ...
  def to_s(inList)
    ...
    if(isAmountEnabled())
      ...
      cols = Array.new
      tempFloat = AgavCutList::float_round_to(@roundToNumOfDigits, @totalBF)
      # the next  line is modified specifically for european users who default to an English version
      # of Sketchup - as Sketchup does not seem to convert these numericals to have comma as the decimal
      # When I find a way to automatically discover which way the user needs to have this, then this can
      # be replaced with some external check - or else decimal to comma can return the string 
      ...
      cols[0] = AgavCutList::decimal_to_comma(tempFloat.to_s)
      component = component + getRow(cols)
      component = component + getFooterRow()
      component = component + getBlankLine()
    end
    ...
    if(isMaterialEnabled())
      ...
      for d in @materialList
        cols = Array.new
        cols[0] = d[0]
	tempFloat =  AgavCutList::float_round_to(@roundToNumOfDigits, d[1])
        # the next  line is modified specifically for european users who default to an English version
	# of Sketchup - as Sketchup does not seem to convert these numericals to have comma as the decimal
	# When I find a way to automatically discover which way the user needs to have this, then this can
        # be replaced with some external check - or else decimal to comma can return the string 
        ...
        cols[1] = AgavCutList::decimal_to_comma(tempFloat.to_s)
        component = component + getRow(cols)
      end ## end for
      component = component + getFooterRow()
      component = component + getBlankLine()
...
class CompactDisplay < Display
  ...
  def getHeadingArray()
    ...
    headings=["Length(L)","Width(W)","Quantity","Part#","Description","Thickness(T)",@measureLabel + " (per)",
...
# same and will be displayed in the compact form
      ...
      cols[3]=c.getName
      cols[4]=getPartPrefix() + AgavCutList::integer_to_fws(3, partId)
      cols[5]=c.getThicknessString
      ...
      roundedBoardFeetTotal =  AgavCutList::float_round_to(@roundToNumOfDigits, (c.getBoardFeet * partCount))
      roundedTotalLengthTotal =  AgavCutList::float_round_to(@roundToNumOfDigits, (c.getTotalLength * partCount))
      cols[7]=AgavCutList::decimal_to_comma(roundedBoardFeetTotal.to_s)
      cols[8]=AgavCutList::decimal_to_comma(roundedTotalLengthTotal.to_s)
  ...
  def processRows(inList)
    ...
    if(inList.parts.length > 0)
      for e in 0..(inList.parts.length-1)
        cols[0]=getPartPrefix() + AgavCutList::integer_to_fws(3, (e+1))
...
class ClpDisplay < Display
  ...
  def processRows(inList)
    ...
    for c in inList
      ...
      cols[4]=AgavCutList::string_to_clp(c.convertMeasureForCLP(c.getThickness).to_s)
      cols[5]=AgavCutList::string_to_clp(c.convertMeasureForCLP(c.getWidth).to_s)
      cols[6]=AgavCutList::string_to_clp(c.convertMeasureForCLP(c.getLength).to_s)
...
class CompactClpDisplay < CompactDisplay
  ...
  def processRows(inList)
    ...
    for c in inList
      ...
      cols[0]=getPartPrefix() + AgavCutList::integer_to_fws(3, partId)
      cols[1]=c.getSubAssemblyName
      cols[2]=c.getName
  ...
  def processRows(inList)
    ...
    if(inList.parts.length > 0)
      for p in 0..(inList.parts.length-1)
        cols[0]= getPartPrefix() + AgavCutList::integer_to_fws(3, (p+1))

...
module PartsIntelligence
  ...
  def partsWiderThanBoards?
    ...
    puts "widest Board Available=" + @widestBoard.to_s if AgavCutList.verbose1
  ...
  def partsThickerThanBoards?
    ...
    puts "thickest Board Available=" + @thickestBoard.to_s if AgavCutList.verbose1
    partArray = @inPartList.getList
  ...
  def normalizeGlueUpParts(thickestBoard)
    ...
    partArray.each { |part| 
      ...
      if part.getThickness.inch > thickestBoard.inch
        ...
        puts "part=" + part.getName + " thickness=" + part.getThickness.inch.to_s + " splits=" + splitInto.to_s + "new thickness=" + splitThickness.to_s if AgavCutList.verbose1
    ...
    # sort the list again
    @inPartList.sort
    puts @inPartList.inspect if AgavCutList.verbose
  ...
  def normalizePartsList(widestBoard)
    ...
    partArray.each { |part| 
      if part.getWidth.inch> widestBoard.inch
          ...
          puts "part=" + part.getName.to_s + " width=" + part.getWidth.to_s + " splits=" + splitInto.to_s + " new width=" + splitWidth.to_s if AgavCutList.verbose1
...
class LayoutEngine  
  ...
  def getBoard
    ...
    puts "Retrieved board from warehouse:" + @board.to_s if AgavCutList.verbosePartPlacement
    return @board
  end
  
  def placeParts
    puts "-----------------------------------------------------" if AgavCutList.verbosePartPlacement
    puts "placeParts" if AgavCutList.verbosePartPlacement
    @remainingPartsToPlace = @inPartList.count
    puts "parts to place=" + @remainingPartsToPlace.to_s if AgavCutList.verbosePartPlacement
    ...
    if board != nil
      ...
      while !@inPartList.empty?
        ...
        if ( layoutBoard.addPartToBoard(nextPart) == false )
          ...
          puts "part did not fit" if AgavCutList.verbosePartPlacement
          @partsToDoList.add(nextPart)
        else
          puts "Part placed: " + nextPart.to_s if AgavCutList.verbosePartPlacement
        end
      end
      ...
      puts "parts left to place=" +  @inPartList.count.to_s if AgavCutList.verbosePartPlacement
      if @remainingPartsToPlace == @inPartList.count
        ...
        puts "Convergence impossible - remaining parts count has not decremented with a new board" if AgavCutList.verbosePartPlacement
        # Nothing was added to this board so remove it from our list
        puts "returning last board to warehouse" if AgavCutList.verbosePartPlacement
        @layoutBoards.pop()
        return
      else
        # this board is full because we've tried every part and no more parts fit but we know we added some
        puts "Board complete or full" if AgavCutList.verbosePartPlacement
        puts "-----------------------------------------------------" if AgavCutList.verbosePartPlacement
  ...
  def run
    ...
    while (@homerDepot.boardsAvailable? && !@inPartList.empty? && !convergenceImpossible?)
      ...
      puts  "boards?=" + @homerDepot.boardsAvailable?.to_s + \
            ...
            " converging?=" + (!@convergenceImpossible).to_s if AgavCutList.verbosePartPlacement
    end
    ...
    puts "returning boards=" + @layoutBoards.size.to_s + "Parts leftover=" + @inPartList.count.to_s if AgavCutList.verbosePartPlacement

...
# 		determine amount of debugging output to the ruby console
		def AgavCutList.verbose1
			false # minimal progress tracking
		end
		
		def AgavCutList.verbose
		...
		def AgavCutList.decimalSeparator
		...
		def AgavCutList.initialiseDecimalNotation
			@@decimalNotation = AgavCutList.decimalSeparator
		...
		def AgavCutList.decimalNotationInitialised?
			@@decimalNotation != ""
		end
	    
		def AgavCutList.decimalNotation
			AgavCutList::initialiseDecimalNotation if !AgavCutList::decimalNotationInitialised?
			@@decimalNotation
		end
		
		# method to round a Float to x digits
		def AgavCutList.float_round_to(x, float)
		...
		def AgavCutList.integer_to_fws(width, integer)
		...
		def AgavCutList.string_to_html(string)
		...
		def AgavCutList.string_to_clp(string)
		    val = string.gsub(/["]/,"\"\"")
		    #val = val.gsub(/[~]/,"")
		end
		...
		def AgavCutList.string_to_csv(string)
		#   1. remove the ~
		...
		def AgavCutList.decimal_to_comma(string)
		# convert english numerical represention of decimal "." to european "," if required
			pattern = /(\d+)(\.)(\d+)/
			match = string.match pattern
			if (match )
				val = string.gsub(/[.]/,"#{AgavCutList::decimalNotation}")
				#DEBUG
		...
		def AgavCutList.group_definition(group)
		...
		def  AgavCutList.version
			return "v0.0.0"
		end
		
		# Cutlist release year
		def AgavCutList.year
			return "2020"
		end	
		
		# Cutlist author
		def AgavCutList.author
			return "agav"
		end	
		
		# Cutlist short description
		def AgavCutList.short_description
